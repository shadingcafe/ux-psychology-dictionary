...
interface Props {
  result: AnalysisResult & { screenshotBase64?: string };
  onReset: () => void;
}
  ...
  return (
    <div className="space-y-8">
        ...
        <button
          onClick={() => {
            const shareUrl = result.id
              ? `${window.location.origin}/analyze/${result.id}`
      ...
      <div className="text-center pt-4">
        <button
          onClick={onReset}
          className="px-6 py-3 bg-teal-400 text-zinc-950 font-medium rounded-lg text-body-sm transition-all hover:bg-teal-300"
        >
          別のサイトを診断する
        </button>
      </div>
